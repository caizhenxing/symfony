...
//if ( count(get_included_files())>1 || php_sapi_name()=='cli' || empty($_SERVER['REMOTE_ADDR']) ) { return; }  // weak block against indirect access
...
if (isset($_GET['RESET']) && !empty($_GET['RESET']) ) {	
	...
	header( 'Location: '.str_replace('?'.(isset($_SERVER['QUERY_STRING'])?$_SERVER['QUERY_STRING']:""),'',(isset($_SERVER['REQUEST_URI'])?$_SERVER['REQUEST_URI']:"")) ); 
	return;
}

if ( isset($_GET['RECHECK']) && !empty($_GET['RECHECK']) ) {
...
if ( empty($_GET['ALL']) ) { meta_display(); echo "</div>";return; }
...
function files_display() {			
	...
	if ( empty($status['scripts']) ) {return true;}
	if ( isset($_GET['DUMP']) ) { print_table($status['scripts']); return false;}
    	$time=time(); $sort=0; 
